...
- **Column generator plugins**: Custom column types you pass to the config builder's [add_column](/code-reference/config/config-builder#data_designer.config.config_builder.DataDesignerConfigBuilder.add_column) method.
...
- **Processor plugins**: Custom processors that transform data before batches, after batches, or after generation completes. Pass them to the config builder's [add_processor](/code-reference/config/config-builder#data_designer.config.config_builder.DataDesignerConfigBuilder.add_processor) method.
